...
export default function UploadFotoForm({ orderId }) {
  ...
  async function handleFile(e) {
    ...
    try {
      ...
      setErr(e2.message || "Terjadi kesalahan saat mengunggah.");
    } finally {
      setBusy(false);
  ...
  return (
    <div>
      ...
      />
      <label
        htmlFor={`foto-${orderId}`}
        className="rk-btn rk-btn-primary"
        style={{ padding: "11px 18px", fontSize: 13.5, cursor: busy ? "wait" : "pointer", opacity: busy ? 0.7 : 1 }}
      >
